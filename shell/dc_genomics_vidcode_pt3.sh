...
# bwa: create index of reference
bwa index data_process/ecoli_rel606.fasta

# bwa: align with bwa mem
...
# automate
for infile in data_process/*_1.fastq
do
  ...
  trimmomatic PE -threads 4 data_process/${base}_1.fastq data_process/${base}_2.fastq \
    ...
    SLIDINGWINDOW:4:20 MINLEN:25 ILLUMINACLIP:data_process/NexteraPE-PE.fa:2:40:15
  
  bwa mem -t 4 data_process/ecoli_rel606.fasta \
    data_process/${base}_1.trim.fastq data_process/${base}_2.trim.fastq \
    > results_process/sam/${base}.sam
done
